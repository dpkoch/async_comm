...
def run_test(target, branch):
    ...
    generate_prerelease_script.py \\
      ...
      --custom-repo \\
      async_com__custom-1:git:https://github.com/dpkoch/async_comm.git:%(branch)s \\
      --level 0 \\
      --output-dir ./
    """ % config
...
def get_prerelease_targets(distros):
    targets = []
    index_yaml = yaml.safe_load(get_repo_file(INDEX_FILE))
    for ros_distro in distros:
        ...
        for file_path in config_files.values():
            config_yaml = yaml.safe_load(get_repo_file(file_path))
